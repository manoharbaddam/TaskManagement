...
const login = () =>{
    ...
    const handleSubmit = async (e) =>{
        ...
        try {
            ...
            if(err.response && err.repsonse.data && err.response.data.message){
                setError(err.response.data.message.detail || 'Invalid email or password');
            } else{
...
const styles = {
    ...
    card: { width: '100%', maxWidth: '400px', padding: '2rem', backgroundColor: 'white', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' },
    ...
    button: { padding: '0.75rem', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '4px', fontSize: '1rem', cursor: 'pointer' },
    errorBox: { backgroundColor: '#fee2e2', color: '#b91c1c', padding: '0.75rem', borderRadius: '4px', marginTop: '1rem', fontSize: '0.875rem' },
    footerText: { marginTop: '1.5rem', textAlign: 'center', fontSize: '0.875rem' }
};
